...
void BrakeController::set_rotation_speed(int speed) {
    m_rotation_speed = std::clamp(speed,
        glidestop::constants::MIN_ROTATION_SPEED,
        glidestop::constants::MAX_ROTATION_SPEED);

    std::string log_msg = "GlideStop: Set rotation speed to " +
                          std::to_string(m_rotation_speed) + " kt\n";
    XPLMDebugString(log_msg.c_str());
}

int BrakeController::get_rotation_speed() const {
...
float BrakeController::get_airspeed_factor() const {
    ...
    float rotation_speed = static_cast<float>(m_rotation_speed);
